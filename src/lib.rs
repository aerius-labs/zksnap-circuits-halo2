...
use snark_verifier_sdk::{
    ...
    halo2::{
        ...
        gen_dummy_snark, gen_dummy_snark_from_vk, gen_snark, gen_snark_shplonk,
    },
    snark_verifier::{
        loader::{
...
impl RecursionCircuit {
    pub fn new(
        ...
        let succinct_verify = |snark: &Snark| {
            let mut transcript =
                PoseidonTranscript::<NativeLoader, _>::new::<0>(snark.proof.as_slice());
    ...
    fn build(&mut self) {
        ...
        let (mut app_instances, app_accumulators) =
            succinct_verify(&self.svk, &loader, &self.voter_snark, None);
        ...
        for (lhs, rhs) in [
            ...
            (
                ...
                &previous_instances[14],
            ),
            // Verify round is increased by 1 when not at first round
            (
                &round,
                &main_gate.add(ctx, not_first_round, previous_instances[15]),
            ),
        ] {
    ...
    fn initial_snark(
        ...
        let mut snark =
            gen_dummy_snark::<RecursionCircuit, As>(params, vk, vec![4 * LIMBS + 4], config_params);
        let g = params.get_g();
