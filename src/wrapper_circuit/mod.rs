...
mod vertical {
    ...
    impl recursion::RecursionExt for App {
        type Input = Fr;

        fn to_fe(input: Self::Input) -> Fr {
            input
        }

        fn to_fe_fe(_: Self::Input) -> (Fr, Fr) {
            unimplemented!()
        }

        fn from_fe(input: Fr) -> Self::Input {
            input
        }

        fn from_fe_fe(_: (Fr, Fr)) -> Self::Input {
            unimplemented!()
        }

        fn new(params: BaseCircuitParams, val: Self::Input) -> Self {
            let mut builder = BaseCircuitBuilder::new(false).use_params(params);
            let val = builder.main(0).load_witness(val);
            builder.assigned_instances[0].extend([val]);

            Self { builder }
        }
    }

    impl Circuit<Fr> for App {
    ...
    impl CircuitExt<Fr> for App {
        ...
        fn instances(&self) -> Vec<Vec<Fr>> {
            vec![
                self.builder.assigned_instances[0]
                    .iter()
...
mod state_transition {
    ...
    impl recursion::RecursionExt for Add {
        type Input = (Fr, Fr);

        fn to_fe(_: Self::Input) -> Fr {
            unimplemented!()
        }

        fn to_fe_fe(input: Self::Input) -> (Fr, Fr) {
            input
        }

        fn from_fe(_: Fr) -> Self::Input {
            unimplemented!()
        }

        fn from_fe_fe(input: (Fr, Fr)) -> Self::Input {
            input
        }

        fn new(params: BaseCircuitParams, input: Self::Input) -> Self {
            let mut builder = BaseCircuitBuilder::new(false).use_params(params);
            let gate = GateChip::<Fr>::default();

            let lhs = builder.main(0).load_witness(input.0);
            let rhs = builder.main(0).load_witness(input.1);
            let result = gate.add(builder.main(0), lhs, rhs);

            builder.assigned_instances[0].extend([lhs, rhs, result]);

            Self { builder }
        }
    }

    impl Circuit<Fr> for Add {
        type Config = BaseConfig<Fr>;
        type FloorPlanner = SimpleFloorPlanner;
        type Params = BaseCircuitParams;
...
mod recursion {
    ...
    pub trait RecursionExt {
        ...
        fn new(params: BaseCircuitParams, state: Self::Input) -> Self;
    }
    ...
    impl RecursionCircuit {
        ...
        pub fn new(
            ...
            state_transition: Snark,
            previous: Snark,
            initial_app_state: <App as RecursionExt>::Input,
            current_app_state: <App as RecursionExt>::Input,
            round: usize,
    ...
    pub fn gen_recursion_snark<
        Vertical: CircuitExt<Fr> + RecursionExt,
        StateTransition: CircuitExt<Fr> + RecursionExt
    >(
        vertical_params: &ParamsKZG<Bn256>,
        state_transition_params: &ParamsKZG<Bn256>,
        recursion_params: &ParamsKZG<Bn256>,
        ...
        initial_state: StateTransition::Input,
        ...
        println!("init app state: {:?}", Vertical::to_fe(app_state.clone()));
