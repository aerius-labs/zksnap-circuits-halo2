...
pub struct EncryptionPublicKeyU32<F: BigPrimeField> {
    pub n: F,
    pub g: F,
}
...
pub struct BaseCircuitInput<F: BigPrimeField> {
    ...
    pub pk_enc: EncryptionPublicKeyU32<F>,
}
...
pub fn base_circuit<F: BigPrimeField>(
    ...
    let vote_enc_old: Vec<AssignedValue<F>> = vote_enc_old_bu
        ...
        .collect();
    println!("vote_enc_old base circuit ={:?}", vote_enc_old_bu);
    let vote_enc_new_bu: Vec<BigUint> = input
        .vote_enc_new
        .iter()
        .map(|x| BigUint::from_slice(x))
        .collect();
    println!("vote_enc base circuit ={:?}", vote_enc_new_bu);
